...
import { Firestore, collection, getDocs, updateDoc, doc, DocumentData, arrayUnion, arrayRemove, deleteDoc } from '@angular/fire/firestore';
...
export class RedComponent {
  ...
  async deletePost(post: any): Promise<void> {
    const postRef = doc(this.firestore, `posts/${post.id}`);
    await deleteDoc(postRef); // Eliminar la publicación de Firestore
    this.posts$ = this.posts$.pipe(
      map(posts => posts.filter((p: any) => p.id !== post.id)) // Filtrar la publicación eliminada
    );
  }
}
